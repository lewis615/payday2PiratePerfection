...
function TextInput:create_gui()
	...
	input_panel:rect( { name = "focus_indicator", visible = true, color = black:with_alpha( 0.1 ), layer = 4 } )
	...
	local text_input = input_panel:text( { name = "input_text", text = "", font = pd2_small_font, font_size = pd2_small_font_size, x = 0, y = 0,
										...
										color = white, layer = 5, wrap = true, word_wrap = false } )
										
	local caret = input_panel:rect( { name="caret", layer = 2, x = 0, y = 2, w = 0.8, h = panel:h() - 5, color = Color(0.05, 1, 1, 1) } )
	input_panel:rect( { name="input_bg", color=black:with_alpha(0.5), layer = -1, valign = "grow", h = input_panel:h() } )
