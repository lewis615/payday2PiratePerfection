...
function MultiChoice:init( panel, button )
	...
	self.index = (--[[ pp_config[ self.name ] and self:index_from_value() or]] val and index_from_value( self, val ) or button.index ) or 1
...
function MultiChoice:create_gui()
	...
	local text_panel = multi_choice:text( { name = "text", layer = 1, wrap = "true", word_wrap = "true", visible = true,
										font = pd2_medium_font, font_size = 20, color = white,
										align="left", halign="left", vertical="center", valign="center", blend_mode = "add" } )
	self.text_panel = text_panel
	
	local arrow_left = multi_choice:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, color = white, layer = 2 } )
	local arrow_right = multi_choice:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, rotation = 180, color = white, layer = 2 } )
...
function MultiChoice:update()
	...
	if arrow_left:inside( x, y ) then
		...
		arrow_left:set_color( white )
	end
	
	
	if arrow_right:inside( x, y ) then
		arrow_right:set_color( grey )
...
function MultiChoice:change_option( index )
	...
	self.index = index
	--pp_config[ self.name ] = data.value
