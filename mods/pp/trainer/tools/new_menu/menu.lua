...
pp_require 'trainer/tools/new_menu/tickbox'
pp_require 'trainer/tools/new_menu/slider'
pp_require 'trainer/tools/new_menu/multi_choice'
pp_require 'trainer/tools/new_menu/text_input'
pp_require 'trainer/tools/new_menu/save_button'
...
--Tweak data
local description_font_size = 20
...
function Menu:create_menu()
	...
	local w_mul = data.w_mul or 3.2
	local h_mul = data.h_mul or 2.5
	local w = scaled_size.width/w_mul--2.8
...
function Menu:add_title()
	...
	local title_text = main:text( { name = "title", text = title or "", layer = 1, wrap = false, word_wrap = false, visible = true,
								   font = T_menu.pd2_large_font, font_size = 28,
...
function Menu:add_description()
	...
	local desc_panel = main:text( { name = "description", text = description or "", layer = 1, wrap = true, word_wrap = true, visible = true,
							   font = T_menu.pd2_medium_font, font_size = description_font_size,
...
function Menu:add_lines() -- add top and bottom lines
	...
	local top_line = main:bitmap( { name = "top_line", texture = "guis/textures/headershadow", layer = 0, color = col_white, w = w } )
	top_line:set_bottom( th + dh + 2 )
	self.bottom_line = main:bitmap( { name = "bottom_line", texture = "guis/textures/headershadow", rotation = 180, layer = 0, color = col_white, w = w } )
end

function Menu:add_info_area()
	local main = self.main
	
	self.info_area = self.main:panel( { name="info_area", x = 0, y = 0, w = main:w(), h = main:h(), layer = 0 } )
	local info_bg = self.info_area:rect( { name="info_bg", layer=0, color=col_black, alpha=0.7, halign="grow", valign="grow" } ) -- background
end
...
function Menu:add_buttons()
	...
	local buttons_panel = info_area:panel( { name = "buttons_panel", x = 10, w = 100, h = info_area:h(), layer = 1 } )
	...
	local text_config = { layer = 2, wrap = "true", word_wrap = "true", visible = true,
						  font = T_menu.pd2_medium_font, font_size = 22, color = col_white,
	...
	local red = col_red
	local white = col_white
	local plug_path = self._data.plugin_path
	for i, button in ipairs( button_list ) do
		local have_plugin = button.plugin
		local selected_path = button.plugin_path or plug_path
		local selected_color = button_stage_3
		...
		if button.menu then
			button_panel:bitmap( { texture = "guis/textures/pd2/crimenet_legend_join", color = white, y = 3, x = button_panel:w() - 20 } )
		end
		
		if button.box then
			button_panel:bitmap( { texture = "guis/textures/pd2/blackmarket/inv_newdrop", color = white, y = 3, x = button_panel:w() - 20 } )
		end
...
function Menu:add_navigation()
	...
	local text_config = { layer = 2, wrap = "true", word_wrap = "true", visible = true,
						  font = T_menu.pd2_medium_font, font_size = 20,
						  align="left", halign="left", vertical="top", valign="top", blend_mode = "add" }
