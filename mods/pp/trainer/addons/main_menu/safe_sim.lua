...
local function choose_item(safe)
	local r_index_over = safe.content == "overkill_01" and {"rare", "epic", "legendary"} or nil
	local data = {amount = 1, category = "weapon_skins"}
	local now = os.date("!*t")
	randseed(now.yday * (now.hour + 1) * (now.min + 1) * (now.sec + 1))
	data.bonus = rand(100) <= (togg_vars["sim_stat"] or sim_chances.stat)
	local rarity = random_choice(r_index_over or r_index, "r")
	local weap_index = {}
	if rarity == "legendary" then
		for _, skin in pairs(T_E_contents[T_E_contents[safe.content].contains.contents[1]].contains.weapon_skins) do
			tab_insert(weap_index, skin)
		end
	else
		for _, skin in pairs(T_E_contents[safe.content].contains.weapon_skins) do
			if T_B_weapon_skins[skin].rarity == rarity then
				tab_insert(weap_index, skin)
			end
		end
	end
	data.entry = weap_index[rand(#weap_index)]
...
local function start_open(name, data)
	...
	if togg_vars.sim_add then
		M_blackmarket:tradable_add_item(item.instance_id, "weapon_skins", item.entry, item.quality, item.bonus, 1)
	end
end

chance_menu = function()
	local r_total = get_total(r_index, "r")
	local data = {{text = tr.safe_sim_rarity..r_total, switch_back = true}}
	for i, r in pairs(r_index) do
	...
	tab_insert(data, {})
	tab_insert(data, {text = tr.reset, callback = function() for _, t in pairs({r_index, q_index}) do for _, i in pairs(t) do togg_vars["sim_"..i] = nil end end togg_vars["sim_stat"] = nil chance_menu() end})
