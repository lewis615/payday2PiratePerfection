...
function check_wall( _unit )
	...
	local check = unit and ( pp_config.ShootThroughWalls and true or not unit:raycast( "ray", unit:movement():m_com(), managers.viewport:get_current_camera_position(), "slot_mask", managers.slot:get_mask( "world_geometry" ), "report" ) )
	return check
end
...
function auto_shoot( player )
	...
	if pp_config.AimbotInfAmmo then
	...
	local damage_mul = pp_config.AimbotDamageMul or equipped_unit_base:damage_multiplier()
...
function auto_aim( player )
	for _,data in pairs( managers.enemy:all_enemies() ) do
		...
		if team_id == "mobster1" or team_id == "law1" then
			...
			if dist < (pp_config.MaxAimDist or 5000) and check_wall( u ) then
...
function aimbot_update()
	if not pp_config.RightClick or mouse_down(mouse, rmb) then
		local player = pmanager:player_unit()
		if not alive( player ) then
			return
		end

		if pp_config.AimMode ~= 2 and check_wall() then
			auto_shoot( player )
		end

		if pp_config.AimMode ~= 1 then
			auto_aim( player )
		end
	end
end
...
local function start_aimbot()
	...
	if pp_config.ShootThroughWalls and not plugins:g_loaded( "shoot_through_walls" ) then
		plugins:pp_require( 'trainer/addons/charmenu/shoot_through_walls', true )
	end
