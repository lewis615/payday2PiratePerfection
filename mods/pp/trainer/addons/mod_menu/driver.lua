...
local function move_car(pos, rot)
	...
	M_player:warp_to( unit:position() + Vector3(0,0,100), Rotation(0,0,0) )
end
...
local function car_update()
	...
	if not car_speedometer then
		...
		car_speedometer.lbl = car_speedometer.ws:panel():text{ name="lbl_speed" , x = speedometer_x * RenderSettings.resolution.x, y = speedometer_y * RenderSettings.resolution.y, text="", font=tweak_data.menu.pd2_large_font, font_size = speedometer_size * RenderSettings.resolution.x, color = ColWhite, layer=2000 }
	end
...
function MAIN()
	...
	unit = W_spawn_unit( World, Idstring(unit_name), player:position(), Rotation(0,0,0) )
	M_player:warp_to( unit:position() + Vector3(0,0,300), Rotation(0,0,0) )
