...
hijack(backuper, "IngameLobbyMenuState.set_lootdrop", function(o, self, drop_category, drop_item_id)
	...
	M_chat:_receive_message(1, tr.loot_spoof_name, tr.loot_spoof_actual..M_localization:text(T_blackmarket[temp.type_items][temp.item_entry].name_id), Color.green)
	local spoofs = {}
	for typ_n, typ in pairs({safes = T_economy.safes, drills = T_economy.drills}) do
		for id, _ in pairs(typ) do
			if pp_config[typ_n..id] then
