...
pp_require 'trainer/job_stuff/host_game'
...
local def_difficulty = pp_config.jobmenu_def_difficulty
local is_singleplayer = pp_config.jobmenu_singleplayer

local contact_menus = {} --Here will be preloaded contact menus

local main,contact_menu,heist_menu,stage_menu,escapes_menu

--local odd_heists = pp_config.jobmenu_odd_jobs or {}
...
local function change_diff(new)
	pp_config.jobmenu_def_difficulty = new
end
...
local main_menu_data = {
	{ text = tr.job_menu_diff, type = "multi_choice", index = 1, multi_callback =
		function(_,val)
			if (val) then
				pp_config.jobmenu_def_difficulty = val
				def_difficulty = val
			end
		end, 
	...
	callback = function() 
		...
		pp_config.jobmenu_singleplayer = is_singleplayer
	end,
	index = #m_permissions,
	switch_back = true },
	{},
...
--Escapes
tab_insert(main_menu_data, { text = tr.job_menu_escapes, callback = escapes_menu })
