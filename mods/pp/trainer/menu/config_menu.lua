...
local main_menu, create_config_menu, load_config_menu, are_you_sure, delete_config_menu, rename_config_menu, rename_config_input, reload_pp_config, save_settings_create_config, _save_settings_create_config
...
local delete_config = function( name )
	...
	if pp_config.DefaultConfig == name then
		pp_config.DefaultConfig = "default_config"
		pp_config()
	end
end
...
local rename_config = function( name, new_name )
	...
	if pp_config.DefaultConfig == name then
		pp_config.DefaultConfig = new_name
...
local main_menu_data = {
	...
	{ text = tr['wp_title'], callback = pp_require("trainer/menu/waypoints_settings"), menu = true },
	{ text = tr['loc_menu'], callback = pp_require("trainer/menu/pre-game/loc_menu"), menu = true },
	{},
	...
	{},
	{ text = tr['config_save_default'], callback = reload_pp_config },
...
main_menu = function()
	Menu_open( Menu, { title = tr['config_menu'], description = tr['config_current']..' - '..pp_config.DefaultConfig, button_list = main_menu_data } )
end
