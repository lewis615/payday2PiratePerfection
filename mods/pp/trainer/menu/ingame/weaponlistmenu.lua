...
pp_require 'trainer/addons/invfix' --In case if it isn't loaded
...
local function swap_weapon( weapon_name, weap_data, custom_blueprint, no_sync )
	...
	if not ( custom_blueprint ) then
		--Fixes bug, preventing you from changing your weapon
...
weapons_menu = function( category )
	...
	for weapon,wdata in pairs( T_weapon ) do
		if ( not str_find(weapon,'npc') --[[Filtering npc's weapons]] ) then
			local use_data = wdata.use_data
			if ( use_data and use_data.selection_index == selection_index ) then
...
main_menu = function()
	if ( alive( GetPlayerUnit() ) ) then
		local data = {
			...
			{},
			{ text = tr.always_dismember, plugin = "always_dismember" },
		}
