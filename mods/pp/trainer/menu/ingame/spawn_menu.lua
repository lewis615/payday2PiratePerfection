...
pp_require 'trainer/other/all_units'
...
local bound_key = pp_config.SpawnUnitKey or '7' --Key, to what spawn unit will be bound
...
local set_anim = function( unit, unit_type )
	local variant = unit_type == 'civilians' and ( pp_config.SpawnCivsAnim or "cm_sp_stand_idle" ) or ( pp_config.SpawnEnemyAnim or "idle" )
...
local set_spawn_anim = function( anim_name, anim_type )
	if anim_type == 'civilians' then
		pp_config.SpawnCivsAnim = anim_name 
	elseif anim_type == 'enemies' then
		pp_config.SpawnEnemyAnim = anim_name
	else
		pp_config.SpawnEnemyAnim = anim_name 
		pp_config.SpawnCivsAnim = anim_name
	end
	...
	if AIState then
		if unit_type == 'friendly' then
			if not plugins:g_loaded( "inf_converts" ) then
				plugins:pp_require( 'trainer/addons/stealthmenu/inf_converts', true )
			end
...
local get_spawn_pos = function()
	local spawn_pos = pp_config.SpawnPos
	local pos
...
local spawn_unit = function( unit_name, unit_type )
	...
	for i = 1, pp_config.SpawnUnitsAmount do
...
local spawn_gage_package = function()
	local SpawnGagePackage = pp_require("trainer/addons/spawngagepackage")
	SpawnGagePackage()
end
...
local spawn_set_amount = function( amount )
	pp_config.SpawnUnitsAmount = amount
end
...
spawn_position_menu = function()
	local data = { 
		{ text = tr['spawn_on_spawn_point'], callback = function() pp_config.SpawnPos = "spawn_point" end, switch_back = main_menu },
		{ text = tr['spawn_on_ray'], callback = function() pp_config.SpawnPos = "ray" end, switch_back = main_menu },
		{ text = tr['spawn_on_random_point'], callback = function() pp_config.SpawnPos = "random_spawn_point" end, switch_back = main_menu },
	}
