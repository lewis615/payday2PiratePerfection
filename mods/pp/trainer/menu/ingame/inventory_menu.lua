...
pp_require('trainer/addons/inventory_menu/rainbags')
...
local bound_key = pp_config.SpawnBagKey or '8' --Key, to what spawn bag will be bound
...
local spawn_bag = function( name )
	...
	if togg_vars.is_rain then
		return RainBags(name, pp_config.rain_bags_amount)
	end
	_spawn_bag(name, pp_config.SpawnBagsAmount or 1)
end
...
modifiers_menu = function()
	local data = {
		...
		--{ text = tr.sync_bag_throw_force, plugin = 'sync_bag_throw_force', host_only = true, switch_back = true },
