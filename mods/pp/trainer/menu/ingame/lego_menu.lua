...
local delete_key = pp_config.LegoDeleteKey or 'h'
--local id_delete = Idstring( delete_key )
local spawn_key = pp_config.LegoSpawnKey or '6'
--local id_spawn = Idstring( spawn_key )
local switch_prev_key = pp_config.LegoPrevKey or '7'
--local id_prev_key = Idstring(switch_prev_key)
local switch_next_key = pp_config.LegoNextKey or '8'
...
local lego_save = function()
	local file_name = pp_config.LegoFile
	local file = open_file("trainer/addons/lego/".. file_name ..".lua", 'w')
...
local delete_favorite = function( unit_name )
	...
	local text = ""
	for line in pp_io.lines( file_name ) do
...
local favorite_menu = function()
	...
	for line in pp_io.lines("trainer/configs/favorites/props.txt") do
		...
		for spawn_node in unit_package_data:model_script_data():children() do
			...
			if find(str_spawn_node, "<sequence_manager file=") and not find(str_spawn_node, "characters") and unit_package_data:network_sync() == "spawn" then
				...
				if Idstring( spawn_unit_name ) == unit_ids then
					i = i + 1
					insert(prop_data, { text = gsub(prop_name, "_", " "), callback = place_lego, alt_callback = function() unit_menu( spawn_unit_name, i ) end, data = { spawn_unit_name, i } })
					break
				end
			end
		end
...
lego_choose_file_menu = function()
	...
	for _,name in pairs( file_list ) do
		insert( data, { text = name, callback = function() pp_config.LegoFile = name end } )
	end
...
main_menu = function()
	if ( not in_chat() ) then
		Menu_open( Menu,
			{ title = tr['lego_menu'],
				description = "Your current file - '".. pp_config.LegoFile ..
