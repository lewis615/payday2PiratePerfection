...
--[[
	Small documentation:
		...
		PluginManager:pp_require( path [, cheap, force, ...] ) : Preloads plugin from filepath. If it is preloaded successfully manager loads it into caller's envirnoment. If "cheap" is true, it will call g_load instead.
...
pp_require 'trainer/experimental/dev/pluginbase'
...
init = function( self )
	...
	self.required = {} --dll's pp_require works not as intended here
end
...
pre_prequire = function( self, path )
	...
	if not was_required then
		...
		if ( not last_plug ) then
			m_log_vs('\n(plugins) failed to pre-pp_require', path)
		end
	else
...
PluginManager.pp_require = prequire
