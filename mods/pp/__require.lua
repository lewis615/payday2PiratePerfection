...
pp_io.open = function( file, mode )
	file = "mods/pp/" .. file
	return io_open( file, mode )
end

pp_io.lines = function( file )
	file = "mods/pp/" .. file
	return io_lines( file )
end

pp_io.io_popen = function( command )
	command = command:gsub("trainer/", "mods/pp/trainer/")
...
do
	...
	if not ( orig__require ) then
		local orig__require = pp_require
		local _G = _G
		...
		local __require_override = {} --Overriden pp_require requests
		...
		--[[
		Arguments:
			(in_path, type: string) : path to the required file, if file wasn't found, it calls original pp_require function, if safe is false/nil
			[safe, type: boolean/anything] : If isn't false or nil, it doesn't call original pp_require function and pcalls loaded chunk, in the result preventing crashes, when you pp_require file from disk.
			[reload, type: boolean/anything] : If isn't false or nil, it ignores was__required check and loads script again. (So it acts like pp_dofile)
		Returns:
			true or non-false/nil result from pp_dofile, if pp_dofile returns anything
		]]
		function pp_require( in_path, safe, reload )
			--Executing clbk on 1st pp_require, function will void itself on execution, result doesn't matter
			first_require_clbk()
			
			--logme('Requring',in_path)
			local path = str_lower(in_path)
			
			--Check if we were required already, if yes, return what returned pp_dofile or true
			local __was_required = was__required[path]
			if (__was_required ~= nil and not reload) then
				return unpack( __was_required )
			end
			--------pp_require function patch-------
			
			--Executing clbks before pp_require
			local before_clbk = __require_pre[path]
			if before_clbk then
				before_clbk()
			end
